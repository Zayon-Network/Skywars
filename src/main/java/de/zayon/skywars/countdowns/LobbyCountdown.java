                ...
                if (counter == 60 ||counter == 30 ||counter == 15 || counter == 10) {
                    ...
                    Bukkit.getScheduler().runTaskAsynchronously(Skywars.getSkywars(), () -> {
                        for (Team team : ZayonAPI.getZayonAPI().getTeamAPI().getRegisteredTeams()) {
                            Location block = Skywars.getSkywars().getLocationConfig().getLocation("config.location.team." + ZayonAPI.getZayonAPI().getTeamAPI().getRegisteredTeams().indexOf(team));
...
//                            public void run() {
                                ...
                                Bukkit.getOnlinePlayers().forEach(player -> {
                                    ...
                                    if (!GameData.getTeamCache().containsKey(player)) {
                                        Team team = ZayonAPI.getZayonAPI().getTeamAPI().addToLowestTeam(player);
